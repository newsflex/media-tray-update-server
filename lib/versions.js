...
function extractChannel(tag) {
  ...
  let extracted = suffix.split(".")[0];
  console.log("extractChannel " + extractChannel + " for " + tag);
  return extracted;
}
...
Versions.prototype.releases = function () {
...
Versions.prototype.filter = function (opts) {
  debug("version filter for ", util.inspect(opts));
    ...
    return _.chain(versions)
      .filter(function (version) {
        ...
        if (
          ...
          debug("Not available for requested paltform");
          return false;
        }
...
Versions.prototype.resolve = function (opts) {
  return this.filter(opts).then(function (versions) {
    ...
    if (!version) {
      throw new Error("Version not found: opts.tag=" + opts.tag);
    }
